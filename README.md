...
**Grady’s Tour:** Travel posts live in `content/gradys-tour/` (CMS collection **Grady’s Tour**) and always publish at `/gradys-tour/<slug>/`. They also appear on the home page in chronological order with everyone else’s posts. Use **Posts** for Eric’s writing (`/posts/<slug>/`); do not put Grady’s travel posts there. `buildFuture = true` in `hugo.toml` so a CMS publish date that is a few minutes ahead of the Cloudflare build still goes live (otherwise Hugo omits the post and the URL falls through to the home page). Optional **Country** on each tour post (Pages CMS select, world list, more than one allowed). A travel post shows a country **chip** on the right of the byline; one country links to `/gradys-tour/?country=france`, several open a dropdown of those links. The Grady’s Tour index has the country dropdown and a rotatable Natural Earth globe (`static/maps/countries-110m.json`, d3-geo orthographic) that aims at writing. Clicking the globe and the dropdown stay in sync. Zoom in / Zoom out (and scroll-wheel zoom) look into the same circle rather than stretching the SVG. The selected country fills dark; countries are not in the tab order (use the dropdown). Countries with posts show a count; picking one filters the list in place and updates the shareable URL. Packing/prep posts can leave Country blank so they only appear on All countries. This is not Leaflet or the Eric's D1 Mission Google Map.
