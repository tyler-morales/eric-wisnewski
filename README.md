...
- **In post body:** Use normal Markdown, e.g. `![alt text](/images/uploads/photo.jpg)`. The render hook turns any image under `/images/uploads/` into a responsive `<picture>` with WebP and multiple widths (480–1280px), plus `loading="lazy"` and `decoding="async"`. Other image URLs are output as a single `<img>` with lazy loading.
...
Content and media are edited via **Pages CMS**. Sign in with GitHub at [https://app.pagescms.org/](https://app.pagescms.org/), open this repository and branch, and use the configured collections (Posts) and media (uploads). Configuration lives in `.pages.yml` at the repo root.
