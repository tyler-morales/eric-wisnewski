...
- **School report card:** Same Posts-only CMS group (`report_card`): School (text) plus Stadium / Fan base / Campus grade dropdowns (A+ through F, and `N/A (A+)`). Fill all four to show a manila paper report card at the **end of the recap** (after the body and gallery, before Share). GPA uses the School Sheets 4.0 scale (`A+` = 4.3, `F` = 0). Local mocks of a CMS save are the `report_card:` blocks on Boston College (D / F / B) and Northern Illinois (C- / D / F) — `hugo server` shows them at `/posts/boston-college/` and `/posts/northern-illinois/`.
