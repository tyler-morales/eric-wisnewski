...
- **School report card:** Same Posts-only CMS group (`report_card`): School, Year, plus Stadium / Fan base / Campus grade dropdowns (A+ through F-, including plus and minus, and `N/A (A+)`). Fill school and all three grades to show a manila paper report card at the **end of the recap** (after the body and gallery, before Share). Year falls back to the publish date if left blank. GPA uses the School Sheets 4.0 scale (`A+` = 4.3, `F+` = 0.3, `F` / `F-` = 0). Local mocks of a CMS save are the `report_card:` blocks on Boston College (D / F / B, 2026) and Northern Illinois (C- / D / F, 2026) — `hugo server` shows them at `/posts/boston-college/` and `/posts/northern-illinois/`.
