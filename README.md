...
2. Run the schema and migrations: `npx wrangler d1 execute blog-comments --remote --file=./migrations/0000_initial_comments.sql`, then `0001_comments_v2.sql`, then `0002_comments_allow.sql`, then `0003_relocate_gradys_tour_comment_urls.sql`, then `0004_newsletter.sql`, then `0005_email_confirm_guard.sql` (or run the SQL in the D1 dashboard).
...
5. **Remove comments:** There is no public Flag button. Open **`/admin/comments/`**, enter `COMMENTS_ADMIN_SECRET`, click **Unlock**, then **Delete** (or **Edit**). Set that secret in Cloudflare Pages → Environment variables (e.g. `openssl rand -hex 32`) and share it with Eric/Grady; never commit it. The admin pages send it as `Authorization: Bearer`, not in the URL. The how-to on that page is the site-facing guide. **Newsletter subscribers** are on **`/admin/subscribers/`** with the same password (Comments / Subscribers links on both pages).

**If comments return 500:** Verify all six migrations have been run against the production D1 database and that the Pages D1 binding uses that database (see step 2 and 3 above). Check Functions logs in the Cloudflare dashboard for the underlying error.
...
If someone submits an address, the API always returns the same “check your inbox” copy so a stranger cannot tell whether that address is already on a list. New signups hide the form and show a two-step tracker (Email → Confirm) when a confirmation email was actually sent. That reminder stays on the subscribe block on later pages until they click the confirmation link (or Use a different email). After a signup or a visit to **manage preferences**, this browser remembers the lists they chose and checks those boxes on every page (not only the author of the current post). They stay **pending** and get no new-post emails until they confirm. Repeat signups to the same address wait 24 hours before another confirmation email. Broken or expired confirm links go to `/subscribe/invalid/` instead of saying the person is confirmed. New-post emails include an **Unsubscribe or manage email preferences** link to `/subscribe/manage/?token=…`, where they can turn lists off or request new ones. Checking a new list from that page sends a confirmation email to that inbox — the manage token cannot skip confirm. Mail-client one-click unsubscribe (`List-Unsubscribe`) still drops that list immediately.
