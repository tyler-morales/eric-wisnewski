...
**Jer’s Prospect Profiles:** Jeremy Bryan’s posts live in `content/jers-prospect-profiles/` (CMS collection **Jer’s Prospect Profiles**) at `/jers-prospect-profiles/<slug>/`. Same gate as Tad: the nav tab (next to Da Breakdown w Tad) and the newsletter checkbox stay off until the first post in that folder is published (`draft: false`). New posts in this collection default to draft.
...
1. Create a new file under `content/posts/` (Eric / home page), `content/gradys-tour/` (Grady’s travel posts), `content/da-breakdown-w-tad/` (Tad’s posts), or `content/jers-prospect-profiles/` (Jeremy’s posts), e.g. `content/posts/my-new-post.md`.
...
- `nav_jers_prospect_profiles` — URL for the "Jer’s Prospect Profiles" link (default: `/jers-prospect-profiles/`). The link is omitted until that section has a published post.
...
Content and media are edited via **Pages CMS**. Eric signs in with **email** (magic link sent to his inbox; invite him by email in the CMS if needed). Maintainers can use GitHub at [https://app.pagescms.org/](https://app.pagescms.org/); open this repository and branch, and use the configured collections (**Posts** for the home page, **Grady’s Tour** for travel posts, **Da Breakdown w Tad** for Tad’s posts, **Jer’s Prospect Profiles** for Jeremy’s posts, **Site updates** for the footer Updates log) and media (uploads). The post **Gallery** field accepts several photos at once; **Body** is a rich-text editor (format text, links, slash commands). Paste a YouTube URL on its own line to embed the video. Configuration lives in `.pages.yml` at the repo root. Collection filenames are `{fields.slug}.md` (not `{{slug}}.md`) so a new post saves as `hello-again.md`, not `}.md`.
...
2. Run the schema and migrations: `npx wrangler d1 execute blog-comments --remote --file=./migrations/0000_initial_comments.sql`, then `0001_comments_v2.sql`, then `0002_comments_allow.sql`, then `0003_relocate_gradys_tour_comment_urls.sql`, then `0004_newsletter.sql`, then `0005_email_confirm_guard.sql`, then `0006_comment_likes.sql`, then `0007_tad_newsletter_list.sql`, then `0008_jer_newsletter_list.sql` (or run the SQL in the D1 dashboard).
...
**If comments return 500:** Verify all nine migrations have been run against the production D1 database and that the Pages D1 binding uses that database (see step 2 and 3 above). Check Functions logs in the Cloudflare dashboard for the underlying error.
...
| `WRITER_EMAIL_JERS_PROSPECT_PROFILES` | Jeremy’s inbox for comments on `/jers-prospect-profiles/` |
...
**Go-live:** verify `ericwisnewski.com` in Resend, run migrations `0004`, `0005`, `0007`, and `0008` on D1 (`0007` lets people subscribe to Tad; `0008` lets people subscribe to Jeremy; without them the form returns “Failed to subscribe”), set secrets above (including postal address), deploy. First dispatch run **seeds** existing RSS items without emailing; only new posts email after that. Feeds: `/posts/index.xml`, `/gradys-tour/index.xml`, `/da-breakdown-w-tad/index.xml`, and `/jers-prospect-profiles/index.xml` (not home `/index.xml`). Dispatch accepts `Authorization: Bearer` only (no `?secret=`).
