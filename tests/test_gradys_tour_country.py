...
class CountryHelperTests(unittest.TestCase):
    def test_slug_and_url_success(self) -> None:
        ...
        slop = read_export("TOUCH_SLOP")
        hold = read_export("TOUCH_HOLD_MS")
        self.assertEqual(call_fn("oneFingerIntent", 4, 4, 1000, slop, hold), "pending")
        self.assertEqual(call_fn("oneFingerIntent", 2, 40, 40, slop, hold), "scroll")
        self.assertEqual(call_fn("oneFingerIntent", 40, 2, 40, slop, hold), "rotate")
        self.assertEqual(call_fn("oneFingerIntent", 30, 30, 40, slop, hold), "scroll")
        self.assertEqual(call_fn("oneFingerIntent", 2, 40, hold, slop, hold), "rotate")
        self.assertEqual(call_fn("oneFingerIntent", 2, 2, hold, slop, hold), "pending")
    ...
    def test_unknown_or_empty_country_failure(self) -> None:
        ...
        self.assertEqual(call_fn("oneFingerIntent", "nope", "nope", 0, 12, 220), "pending")
        self.assertEqual(call_fn("oneFingerIntent", None, None, -5, 12, 220), "pending")
...
class CountrySourceTests(unittest.TestCase):
    ...
    def test_templates_put_globe_on_index_and_chip_on_posts_success(self) -> None:
        ...
        self.assertIn("export function oneFingerIntent", js)
        self.assertIn("oneFingerIntent(", js)
        self.assertIn("(hover: none) and (pointer: coarse)", js)
        self.assertIn("pointerType", js)
        self.assertIn("touch-action: none", css)
        self.assertNotIn("touch-action: pan-y", css)
        self.assertRegex(
            css,
            r"@media \(hover: none\) and \(pointer: coarse\) \{\s*"
            r"\.tour-country-map-tools \{\s*display: none;",
        )
        self.assertNotIn("tour-country-map-hint", nav)
        self.assertNotIn("Click a country to filter", nav)
        self.assertNotIn("Tap a country to filter", nav)
        self.assertNotIn("Drag to rotate", nav)
        self.assertNotIn("Drag sideways", nav)
        self.assertNotIn(".tour-country-map-hint", css)
...
class CountryBuildTests(unittest.TestCase):
    ...
    def test_index_globe_filters_and_post_chip_navigates_success(self) -> None:
        ...
        self.assertNotIn("tour-country-map-hint", self.tour)
        self.assertNotIn("Click a country to filter", self.tour)
        self.assertNotIn("Tap a country to filter", self.tour)
        self.assertNotIn("Drag to rotate", self.tour)
        self.assertNotIn("Pinch to zoom", self.tour)
