...
function ctaButtonHtml(cta, variant) {
  if (!cta || !cta.url || !cta.label) return '';
  const href = escapeAttr(cta.url);
  const label = escapeHtml(cta.label);
  const isPrimary = variant === 'primary';
  const tdStyle = isPrimary
    ? 'background-color:#111111;border-radius:8px;'
    : 'background-color:#ffffff;border:1px solid #e5e5e5;border-radius:8px;';
  const aStyle = isPrimary
    ? 'display:inline-block;padding:12px 22px;color:#ffffff;font-family:Arial,Helvetica,sans-serif;font-size:15px;font-weight:700;line-height:1.2;text-align:center;text-decoration:none;'
    : 'display:inline-block;padding:12px 22px;color:#ea580c;font-family:Arial,Helvetica,sans-serif;font-size:15px;font-weight:700;line-height:1.2;text-align:center;text-decoration:none;';
  return `<td class="cta-col" align="center" valign="middle" style="padding:6px;">
<table role="presentation" cellpadding="0" cellspacing="0" border="0"><tr>
<td style="${tdStyle}"><a href="${href}" style="${aStyle}">${label}</a></td>
</tr></table>
</td>`;
}

/**
 * Overreacted-style transactional HTML (tables + inline CSS, no images).
...
export function brandedTransactionalEmail({
  ...
  const primaryHtml = ctaButtonHtml(primaryCta, 'primary');
  const secondaryHtml = ctaButtonHtml(secondaryCta, 'secondary');
  const toLine = to
...
@media only screen and (max-width:480px) {
  .cta-col { display:block !important; width:100% !important; padding:6px 0 !important; }
  .cta-col a { display:block !important; }
}
</style>
</head>
...
<table role="presentation" cellpadding="0" cellspacing="0" border="0" class="cta-row" align="center">
<tr>
${primaryHtml}
${secondaryHtml}
</tr>
  ...
  if (primaryCta && primaryCta.url && primaryCta.label) {
    textLines.push('', `${primaryCta.label}: ${primaryCta.url}`);
  }
  if (secondaryCta && secondaryCta.url && secondaryCta.label) {
    textLines.push(`${secondaryCta.label}: ${secondaryCta.url}`);
  }
