...
// .wrangler/tmp/pages-AqBCSV/functionsWorker-0.4100445431372237.mjs
...
__name2(isValidUrlParam, "isValidUrlParam");
...
async function onRequestPost(context) {
  ...
  if (parentId != null) {
    ...
    const parent = await db.prepare("SELECT id, parent_id FROM comments WHERE id = ? AND url = ?").bind(parentId, url).first();
    if (!parent) {
  ...
  try {
    ...
    const { meta } = await stmt.bind(url, author, email || null, text, parentId, editToken).run();
...
async function onRequestPut(context) {
  ...
  const row = await db.prepare("SELECT id, edit_token FROM comments WHERE id = ?").bind(id).first();
...
// .wrangler/tmp/bundle-u1QZAw/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-u1QZAw/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.4100445431372237.js.map
